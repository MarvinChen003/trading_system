...
def _covert_json_to_pandas_dataframe(stock_codes):
    data_last_year = _get_daily_trades_for_specific_stocks(
        stock_codes, "20190512", "20200514"
    )
...
def _satisfy_ma100_ma20_strategy(stocks_dataframe):
    ...
    for ts_code in ts_codes:
        ...
        date_price_dict = pd.Series(
            stock_dataframe.close.values, index=stock_dataframe.trade_date
        ).to_dict()
        ...
        if (
            all(x in ma100_matrix for x in ["Upon", "Under"])
            and ma100_matrix[-1] == "Upon"
        ):
